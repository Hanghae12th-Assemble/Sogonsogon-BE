...
//        }
    ...
    // 선택된 라디오 조회
    @Transactional
    public RadioResponseDto findRadio(Long radidId) {
    ...
    // 선택된 라디오 정보 조회
    @Transactional
    public RadioResponseDto updateRadio(Long radidId, RadioRequestDto requestDto, UserDetailsImpl userDetails) throws IOException {

        // 라디오 방 유저인지 확인
        Member membername = memberRepository.findByMembername(userDetails.getUsername()).orElseThrow(
