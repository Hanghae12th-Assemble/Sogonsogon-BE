...
//                .userService(customOAuth2MemberService);
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedMethods(Arrays.asList("*")); // 모든 메서드 허용 (GET, POST, PUT, DELETE 등)
        config.setAllowedHeaders(Arrays.asList("*")); // 모든 헤더 허용
        config.setAllowCredentials(true); // 쿠키 요청 허용 (필요한 경우 사용)
        config.addExposedHeader("*");

        UrlBasedCorsConfigurationSource source = new UrlBasedCorsConfigurationSource();
        source.registerCorsConfiguration("/**", config); // 모든 URL에 대해 적용
        return source;
    }
}