...
//            responseDto.setFollowed(true);
    ...
    public FollowResponseDto toggleFollow(String membername, UserDetailsImpl userDetails) {
        ...
        if (followStatus == null) {
            ...
            notificationService.send(follow, AlarmType.eventFollower, follower.getMembername() + "님이 회원님을 팔로우하였니다.", null);
            return FollowResponseDto.of(newFollow);
        } else {
            followRepository.deleteById(followStatus.getId());
            notificationService.send(follow, AlarmType.eventFollower, follower.getMembername() + "님이 회원님을 팔로우 취소하였습니다.", null);
